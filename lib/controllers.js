...
Controllers.updateStats = function(next){
	...
	async.waterfall([
		...
		function(callback){ // get r6stats settings
			console.log('[' + new Date().toISOString() + '][R6STATS] 🔫  RETRIEVING R6STATS SETTINGS')
		...
		function(callback){ //get uids
			console.log('[' + new Date().toISOString() + '][R6STATS] 🔫  RETRIEVING UIDS')
		...
		function(callback){ //get user platform info
			console.log('[' + new Date().toISOString() + '][R6STATS] 🔫  RETRIEVING USER PLATFORM INFORMATION')
		...
		function(callback){ // get users forum info
			console.log('[' + new Date().toISOString() + '][R6STATS] 🔫  RETRIEVING USER FORUM INFORMATION')
			var userKeys = []
		...
		function(callback){ // get R6Stats
			console.log('[' + new Date().toISOString() + '][R6STATS] 🔫  RETRIEVING STATS START')
			async.forEachOf(settings.users, function(user, index, cb){
				R6.stats(user.uplay_id,settings.platform,false).then(response => {
					if(response.hasOwnProperty('player')){
						console.log('[' + new Date().toISOString() + '][R6STATS] 🔫  RETRIEVING STATS FOR  ' + settings.users[index].username + ' ')
				...
				console.log('[' + new Date().toISOString() + '][R6STATS] 🔫  RETRIEVING STATS END')
				callback()
			})
		},
		function(callback){ // get R6Operators
			console.log('[' + new Date().toISOString() + '][R6STATS] 🔫  RETRIEVING OPERATORS START')
			async.forEachOf(settings.users, function(user, index, cb){
				R6.stats(user.uplay_id,settings.platform,true).then(response => {
					if(response.hasOwnProperty('operator_records')){
						// console.log(response)
						console.log('[' + new Date().toISOString() + '][R6STATS] 🔫  RETRIEVING OPERATORS FOR  ' + settings.users[index].username + ' ')
				...
				console.log('[' + new Date().toISOString() + '][R6STATS] 🔫  RETRIEVING OPERATORS END')
				callback()
			})
		},
		function(callback){
		...
		function(callback){
			...
			if(_.isEmpty(settings.validCron.errors) && settings.updateTime != ''){
				cronJobs.push(new cron(settings.updateTime, function(){
					Controllers.updateStats(function(err,result){
						console.log('[' + new Date().toISOString() + '][R6STATS] 🔫  CRON RUN COMPLETE')
					})
				}, null, false));
